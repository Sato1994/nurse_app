    ...
    <v-list dense>
      ...
      <v-list-item-group
        v-for="(offer, index) in formedMyOffers"
          :key="index"
          color="primary">
        <v-list-item @click="createRoom(offer.user.id, offer.formedOffer)">
          <v-list-item-content>
...
export default {
  ...
  computed: {
    formedMyRequests() {
      const requests = this.$store.getters['myInfo/getMyRequests'].map(obj=> {
        ...
        const newObject = {id: obj.id, user: obj.user, formedRequest: `${s.getFullYear()}年${s.getMonth() +1}月${s.getDate()}日${s.getHours()}時${s.getMinutes()}分から${f.getFullYear()}年${f.getMonth() +1}月${f.getDate()}日${f.getHours()}時${f.getMinutes()}分`}
        return newObject
      })
      return requests
    },
    formedMyOffers() {
      const offers = this.$store.getters['myInfo/getMyOffers'].map(obj=> {
        const s = new Date(obj.start_time)
        const f = new Date(obj.finish_time)
        const newObject = {id: obj.id, recruitmentTime: obj.recruitmentTime, user: obj.user, formedOffer: `${s.getFullYear()}年${s.getMonth() +1}月${s.getDate()}日${s.getHours()}時${s.getMinutes()}分から${f.getFullYear()}年${f.getMonth() +1}月${f.getDate()}日${f.getHours()}時${f.getMinutes()}分`}
        return newObject
      })
      return offers
    },
  ...
  methods: {
    createRoom(userID,formedOffer) {
      this.$axios.post(`/api/rooms/user/${userID}`,{},{ headers: this.$cookies.get('authInfo') })
      .then((response) => {
