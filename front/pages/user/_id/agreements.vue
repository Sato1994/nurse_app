<template>
  <v-card>
    ...
    <v-list dense subheader two-line>
      <v-subheader inset>病院からのオファー一覧</v-subheader>
      ...
      <v-subheader inset>病院へのリクエスト一覧</v-subheader>
...
export default {
  ...
  methods: {
    createRoom(requestId, hostID, startTime, finishTime) {
      this.$axios
        .post(
          `/api/rooms/host/${hostID}`,
          {
