  ...
  describe "POST /create" do
    ...
    context "リクエストが成功する場合い" do
      before do
        ...
        post "/api/user_messages/#{room.id}", params: { message: "こんにちは" }, headers: { uid: uid, client: client, "access-token": access_token }
      end
    ...
    context "リクエストが失敗する場合" do
      ...
      it "入力漏れならステータス400を返す" do
        room = create(:room)
        post "/api/user/sign_in", params: { email: room.user.email, password: room.user.password }
        post "/api/user_messages/#{room.id}", params: { message: "  " }, headers: { uid: uid, client: client, "access-token": access_token }
